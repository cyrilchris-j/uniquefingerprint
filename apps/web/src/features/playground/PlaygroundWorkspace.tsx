...
import { Sandbox, SandboxSkeleton } from "./Sandbox.js";
...
export default function Demo() {
  ...
  if (!embedded) {
    useDocumentTitle(
      itemData ? `${itemData.title} — Playground — UniqueFingerprint` : "Playground — UniqueFingerprint",
    );
  }
  ...
  const handleSelect = (name: string) => {
    ...
    if (!embedded) {
      setParams({ item: name }, { replace: true });
    }
  };
    ...
    <div className="grid gap-8 lg:grid-cols-[18.5rem_minmax(0,1fr)] xl:grid-cols-[20.5rem_minmax(0,1fr)] lg:gap-10 xl:gap-14 items-start">
      ...
      <aside aria-label="Choose a resource">
        <div className="flex items-baseline justify-between">
          ...
          <span className="font-mono text-[10px] text-graphite">{filteredOptions.length}</span>
        </div>
        ...
        <div className="relative my-2">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-graphite/60 pointer-events-none" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search 1,038+ items..."
            aria-label="Filter resources"
            className="w-full pl-8 pr-3 py-1.5 text-xs font-mono rounded-md bg-surface/60 border border-line/30 text-ink placeholder:text-graphite/50 focus:outline-none focus:border-oxide/60 transition-colors"
          />
        </div>
